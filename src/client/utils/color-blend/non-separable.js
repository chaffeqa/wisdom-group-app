...
var blendHelpers = {
  ...
  setSat: function(rgb, s) {
    var sortedChannels = Object.keys(rgb).sort(function(a, b) {
      return rgb[a] - rgb[b];
    });
    ...
    if (result[cMax] > result[cMin]) {
      result[cMid] =
        ((result[cMid] - result[cMin]) * s) / (result[cMax] - result[cMin]);
      result[cMax] = s;
    } else {
...
var methods = {
  hue: function(backdrop, source) {
    return blendHelpers.setLum(
      blendHelpers.setSat(source, blendHelpers.sat(backdrop)),
      blendHelpers.lum(backdrop)
    );
  },

  saturation: function(backdrop, source) {
    return blendHelpers.setLum(
      blendHelpers.setSat(backdrop, blendHelpers.sat(source)),
      blendHelpers.lum(backdrop)
    );
  },
...
const nonSeparable = (backdrop, source, mode) => {
  return helpers.convertFromUnit(
    methods[mode](
      helpers.convertToUnit(backdrop),
      helpers.convertToUnit(source)
    )
  );
};
